...
int main(void) {
	...
	// Waiting for the first button push
	//while(!bit_is_set(PINB,BUTTON)) {};
	//delayTenMicroseconds(6000);
	...
	for (;;) {
		for (current_code = 0; current_code < ir_codes_length; ++current_code) {
			...
			// Flash ACT_LED, to show to the user that a code has been sent
			quickLEDFlash();
		}
