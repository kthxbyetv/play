...
ISR(INT0_vect) {
	// Set working to 0, so it will break the loop and make the chip enter in sleep mode
	working = 0;
}
...
int main(void) {
	...
	for(;;) {
		// We start the work, so let's define the variable
		working = 1;
		...
		*/
		...
		// A little delay, otherwise it won't work and the CPU will return in sleep mode immediatly
		delay_ten_us(6000);
